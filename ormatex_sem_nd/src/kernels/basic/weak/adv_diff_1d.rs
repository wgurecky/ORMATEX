...
///
/// Weak form `(nu*u' - vel*u)*v'` with `nu`/`vel` from the material context;
/// the Jacobian adds their state-derivative product-rule terms. Tensor mirror:
/// [`TensorKernelAdvDiff`](crate::kernels::basic::tensor::adv_diff_1d::TensorKernelAdvDiff).
