...
fn main() {
    ...
    let composed_system = FluidSystem::new(&problem, composed_kernel());
    let start = Instant::now();
    ...
    println!(
        "cavity comparison: max |fused-composed|={max_difference:.3e}, fused={fused_runtime:?}, composed={composed_runtime:?}, ratio={ratio:.3}"
    );
