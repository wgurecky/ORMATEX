...
fn tensor_1d_in_place_jacobian_matches_allocating_action() {
    ...
    let kernel = TensorKernelAdvDiff(KernelAdvDiff::new(0.2, -0.4));
...
fn tensor_1d_path_matches_generic_residual_jacobian_and_bilinear() {
    ...
    let tensor_operator = problem.tensor_residual_operator(&tensor);
    ...
    let tensor_matrix = tensor_operator.assemble_jacobian(state.as_ref()).to_dense();
...
fn coefficient_receives_explicit_time_and_space() {
    let problem = SEM1DProblem::new(mesh(1), 2, FieldRegistry::new(["x"]), DofReduction1D::None);
